...
async function passwordResetMail (recieverEmail) {
    ...
    html: `<p>Please, Click this link to Open 👉 <a href='http://localhost:3000/resetPassword?user=${recieverEmail}' >Click Here!</a> 👈 a Page. From there You can change the Password</p>`, 
    });
...
const loadForgotPassword = async (req, res) => {
    try {
        ...
        res.render('forgotPassword', {pageTitle: 'Forgot Password | PhoneZee', loginOrCart: req.session});
...
const loadResetPassword = async (req, res) => {
    try {
        ...
        res.render('resetPassword', {pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
...
const resetPassword = async (req, res) => {
    try {
        ...
        if (req.body.newPassword.trim() === '') {
            res.render('resetPassword', { validationMessage: 'Please enter your password.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (req.body.confirmPassword.trim() === '') {
            res.render('resetPassword', { validationMessage: 'Please enter your password.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (req.body.newPassword.includes(' ')) {
            res.render('resetPassword', { validationMessage: 'Password should not contain spaces.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (req.body.newPassword.length < 8) {
            res.render('resetPassword', { validationMessage: 'Password must be at least 8 characters long.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (!/[A-Z]/.test(req.body.newPassword)) {
            res.render('resetPassword', { validationMessage: 'Password must contain at least one uppercase letter.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (!/[a-z]/.test(req.body.newPassword)) {
            res.render('resetPassword', { validationMessage: 'Password must contain at least one uppercase letter.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (!/\d/.test(req.body.newPassword)) {
            res.render('resetPassword', { validationMessage: 'Password must contain at least one number.' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});
        } else if (!/[@$!%*?&]/.test(req.body.newPassword)) {
            res.render('resetPassword', { validationMessage: 'Password must contain at least one special character (@, $, !, %, *, &, ?)' , pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session});          
        } else {
            ...
            if (newPassword == confirmPassword) {
                ...
                if (!passwordMatch) {
                    ...
                    if (ans) {
                        ...
                        res.render('resetPassword', {pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session, validationMessage: 'An Error Occured While Changing Password'});
                    }
    
                } else {
                    res.render('resetPassword', {pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session, validationMessage: 'New Password can"t be the Old Password'});
                }
    
            } else {
                res.render('resetPassword', {pageTitle: 'Reset Password | PhoneZee', loginOrCart: req.session, validationMessage: 'Incorrect Confirm Password'});
            };
