...
const loadAddProduct = async (req, res) => {
  try {
    ...
    res.render("addProductGeneral", {
      activeProductMessage: "active",
...
const activeOrInactive2 = async (req, res, next) => {
  try {
    ...
    res.status(statusCode.OK).json({
      ...
      newStatus: updatedProduct.is_hide ? "In Active" : "Active",
    });
  } catch (err) {
...
const insertProduct = async (req, res) => {
  try {
    ...
    if (productAlready) {
      return res
        .status(400)
        .json({ message: "Product with name already exists" });
      ...
      return res
        .status(400)
        .json({ message: "Please add a Description for Product" });
    } else if (req.body.productPrice == "") {
      return res.status(400).json({ message: "Add a Price to the Product" });
    } else if (req.body.productStock == "") {
      return res.status(400).json({ message: "Add a Product Stock" });
    } else if (productImages.length < 4) {
      return res.status(400).json({ message: "Add 4 Images" });
    } else if (req.body.productRam == "") {
      return res.status(400).json({ message: "Add the RAM of the device" });
    } else if (req.body.productStorage == "") {
      return res
        .status(400)
        .json({ message: "Add the Storage capacity of the device" });
    } else if (req.body.productStock <= 0) {
      return res.status(400).json({ message: "Enter a Valid Stock" });
    } else if (req.body.productPrice <= 0) {
      return res.status(400).json({ message: "Enter a Valid Price" });
    } else if (req.body.productRam <= 0) {
      return res.status(400).json({ message: "Enter a Valid RAM" });
    } else if (req.body.productStorage <= 0) {
      return res.status(400).json({ message: "Enter a Valid Storage" });
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      ...
      res.render("editProduct", {
        message: "Please make an Description for Product",
        product: product,
        categories: categoryData,
      });
    } else if (req.body.productPrice == "") {
      res.render("editProduct", {
        message: "Add a Price to the Product",
        product: product,
        categories: categoryData,
      });
    } else if (req.body.productRam == "") {
      res.render("editProduct", {
        message: "Add the Ram of the device",
        product: product,
        categories: categoryData,
      });
    } else if (req.body.productStorage == "") {
      res.render("editProduct", {
        message: "Add the Ram capacity of the device",
        product: product,
        categories: categoryData,
      });
    } else if (req.body.productStock == "") {
      res.render("editProduct", {
        message: "Add a Product Stock",
        product: product,
        categories: categoryData,
      });
    }
    ...
    // }
      ...
      res.render("editProduct", {
        message: "Enter a Valid Ram",
        product: product,
        categories: categoryData,
      });
    } else if (req.body.productStorage <= 0) {
      res.render("editProduct", {
        message: "Enter a Valid Storage",
        product: product,
        categories: categoryData,
      });
    } else {
      ...
      const updatedData = {
        ...
        storage: req.body.productStorage,
        product_image:
          productImages.length > 0 ? productImages : product.product_image,
      };
...
const loadProduct = async (req, res) => {
  try {
    ...
    const relatedProductData = await Product.find({
      _id: { $ne: id },
      category: productData.category,
    });
    const cartDataForCount = await Cart.findOne({
